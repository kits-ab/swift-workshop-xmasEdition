...
//  shibun (iOS)
//
//  Created by Tobias Lans on 2020-10-01.
//
...
protocol PersistentStore {
    ...
    func deleteAll(_ entityName: String) -> AnyPublisher<Int, Error>
}
...
struct CoreDataStack: PersistentStore {
    ...
    func update<Result>(_ operation: @escaping DBOperation<Result>) -> AnyPublisher<Result, Error> {
        let update =
            Future<Result, Error> { [weak bgQueue, weak container] promise in
                bgQueue?.async {
    ...
    func deleteAll(_ entityName: String) -> AnyPublisher<Int, Error> {
        let delete = Future<Int, Error> { [weak bgQueue, weak container] promise in
            bgQueue?.async {
                guard let context = container?.newBackgroundContext() else { return }
                context.configureAsUpdateContext()
                context.performAndWait {
                    let deleteFetch = NSFetchRequest<NSFetchRequestResult>(entityName: entityName)
                    let deleteBatch = NSBatchDeleteRequest(fetchRequest: deleteFetch)
                    deleteBatch.resultType = .resultTypeCount
                    do {
                        let batchDeleteResult = try context.execute(deleteBatch) as! NSBatchDeleteResult
                        let removedItems = batchDeleteResult.result! as! Int
                        context.reset()
                        promise(.success(removedItems))
                    } catch {
                        context.reset()
                        promise(.failure(error))
                    }
